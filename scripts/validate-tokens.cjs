...
function scanFile(filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    // Beri komentar `token-ok: <alasan>` di baris yang sama.
    // Dipakai HANYA untuk tempat yang secara teknis tidak bisa memakai
    // var() — misalnya <meta name="theme-color">, yang dibaca oleh sistem
    // operasi sebelum CSS apa pun dimuat.
    if (/token-ok:/.test(line)) {
      return;
    }
